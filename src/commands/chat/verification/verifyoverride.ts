    ...
    async execute(
        ...
        switch (subcommand) {
            ...
            case 'view': {
                ...
                await handleViewSubcommand(interaction, targetUser, interaction.guild!);
                break;
            }
            case 'list': {
                // TODO: support this
...
async function handleViewSubcommand(interaction: ChatInputCommandInteraction, targetUser: User, guild: Guild): Promise<void> {
    try {
        ...
        await interaction.editReply({
            embeds: [new EmbedBuilder().setColor('Red').setDescription('An error occurred while viewing the verification override. Please try again later.')],
        });
    }
}
