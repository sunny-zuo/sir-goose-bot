...
export class UserContextMenuCommandInteractionCreateEventHandler implements EventHandler {
    ...
    async execute(interaction: Interaction): Promise<void> {
        ...
        const command = client.userContextMenuCommands.get(interaction.commandName);

        if (!command || !command.enabled) return;
        if (!command.isUserContextMenuCommand) return;
        if (interaction.guild && interaction.guild.available === false) return;
        if (command.isRateLimited(interaction.user.id)) {
            logger.info(
                {
                    ratelimit: { type: 'userContextMenu', name: command.name },
                    guild: { id: interaction.guild?.id ?? 'none' },
                    user: { id: interaction.user.id },
                },
                'User was ratelimited on a user context menu command interaction'
            );
        ...
        logger.info(
            ...
            `Executing user context menu command ${interaction.commandName}`
        );
