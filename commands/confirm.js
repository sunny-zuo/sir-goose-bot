    ...
    async assignRole(message, user) {
        ...
        if (user.program === guildSettings.verificationProgram) {
            ...
            if (guildSettings.autoGuest) {
                ...
                return message.channel.send(`Error: You're not in ${guildSettings.verificationProgram}, and the server Admins did not enable auto guest verification. Please message an Admin manually for a guest role`);
            }
        }
    }
}